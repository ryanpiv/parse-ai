...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (fromUser.invalid) {
        return res.status(400).json({
            error: 'That does not look like a Claude Console API key (should start with sk-ant-). Create one at https://console.anthropic.com/settings/keys',
        })
    }
    ...
    if (!apiKey) {
        return res.status(401).json({
            error: 'No Claude API key. Paste a Console key in the Claude API key panel (console.anthropic.com). Claude.ai / Pro login is not available for third-party apps.',
        })
    }
