...
export default async function handler(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (req.query.debug === 'hero') {
        ...
        const treeData = await blizzardGet(
            `/data/wow/talent-tree/${m[1]}/playable-specialization/${specId}`,
            'static',
        )
    ...
    try {
        ...
        function parseNode(n: any, type: string) {
            ...
            if (isChoice) {
                ...
                const firstName =
                    ranks[0]?.tooltip?.talent?.name || ranks[0]?.tooltip?.spell_tooltip?.spell?.name || null
                entries = ranks.map((r: any, idx: number) => {
                    ...
                    return {
                        ...
                        name:
                            talentObj.name || st.spell?.name || spellObj.name || firstName || `Node ${n.id}`,
                        description: desc,
        ...
        const bounds = (arr: any[]) =>
            arr.length
                ? {
                      minRow: Math.min(...arr.map((n) => n.row)),
                      maxRow: Math.max(...arr.map((n) => n.row)),
                      minCol: Math.min(...arr.map((n) => n.col)),
                      maxCol: Math.max(...arr.map((n) => n.col)),
                  }
                : null
        ...
        console.log(
            `[blizzard-tree] class=${classNodes.length} spec=${specNodes.length} hero=${heroNodes.length} heroTypes=${heroTypes} edges=${edges.length}`,
        )
        res.json(result)
    } catch (e: any) {
