...
export function collectNames(
    events: Array<{ abilityGameID?: number; ability?: { name?: string } }>,
...
export async function fetchFullFightData({
    reportCode,
    fightStart,
    fightEnd,
    playerId,
    setStep,
}: FetchFightDataParams): Promise<RawFightData> {
    setStep('Fetching cast events...')
    const [castData, buffData, debuffData, damageData, enemyDeathData] = await Promise.all([
        gql(
            `query($c:String!,$s:Float!,$e:Float!,$src:Int!){reportData{report(code:$c){events(dataType:Casts,startTime:$s,endTime:$e,sourceID:$src,limit:10000){data}}}}`,
            { c: reportCode, s: fightStart, e: fightEnd, src: playerId },
        ),
        gql(
            `query($c:String!,$s:Float!,$e:Float!,$tgt:Int!){reportData{report(code:$c){events(dataType:Buffs,startTime:$s,endTime:$e,targetID:$tgt,limit:10000){data}}}}`,
            { c: reportCode, s: fightStart, e: fightEnd, tgt: playerId },
        ),
        gql(
            `query($c:String!,$s:Float!,$e:Float!,$src:Int!){reportData{report(code:$c){events(dataType:Debuffs,startTime:$s,endTime:$e,sourceID:$src,limit:10000){data}}}}`,
            { c: reportCode, s: fightStart, e: fightEnd, src: playerId },
        ),
        gql(
            `query($c:String!,$s:Float!,$e:Float!,$src:Int!){reportData{report(code:$c){events(dataType:DamageDone,startTime:$s,endTime:$e,sourceID:$src,limit:10000){data}}}}`,
            { c: reportCode, s: fightStart, e: fightEnd, src: playerId },
        ),
        gql(
            `query($c:String!,$s:Float!,$e:Float!){reportData{report(code:$c){events(dataType:Deaths,startTime:$s,endTime:$e,limit:10000){data}}}}`,
...
export async function buildAnalyzedFightData({
    ...
    const { getStateAt, buffWindows } = buildStateTracker(buffs, debuffs, fightStart) as {
        getStateAt: any
        buffWindows: Record<number, any[]>
    }
    ...
    const annotated = annotateCasts(playerCasts, {
        getStateAt,
        getDamageAfterCast,
        getNPCDeathsBy,
        fightStart,
