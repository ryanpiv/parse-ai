...
export function fmtBuffUptimes(
    name: string,
    ...
    const entries = Object.entries(uptimes)
        .map(([id, pct]) => ({
            id: Number(id),
            name: nameMap[Number(id)] || `Buff ${id}`,
            pct: pct as number,
        }))
...
export function fmtCastDetails(name: string, annotated: any[], limit = 60): string {
    ...
    annotated.slice(0, limit).forEach((c: any) => {
        ...
        const dmgStr =
            c.totalDamage > 0 ? ` → ${c.totalDamage.toLocaleString()} dmg${c.hitsCrit ? ' (crit)' : ''}` : ''
...
export function fmtPotion(name: string, annotated: any[]): string {
    const potionCasts = annotated.filter((c: any) => c.name && c.name.toLowerCase().includes('potion'))
