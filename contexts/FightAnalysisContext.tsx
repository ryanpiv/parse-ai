...
export function FightAnalysisProvider({ children }: { children: ReactNode }) {
    ...
    const aiLiveRef = useRef({
        start: 0,
        inTok: undefined as number | undefined,
        outTok: undefined as number | undefined,
    })
    ...
    const runAI = useCallback(
        async (
            userMsg: string,
            priorThread: ChatMsg[],
            ctxOverride: string | undefined,
            channel: 'compare' | 'analyze',
        ) => {
            if (channel === 'compare') {
            ...
            const ctx =
                ctxOverride ?? (channel === 'compare' ? buildContextCompare() : buildContextPlayerOneCb())
    ...
    }, [aiLoading, p1data, p2data, soloFromReport, talentDiff, fightKill1, fightKill2, runAI])
    ...
    executeSoloReportFullRef.current = async (code: string, fightId: number, srcRaw: string) => {
        ...
        if (pid == null || Number.isNaN(Number(pid))) {
            throw new Error(
                `Could not find player "${srcRaw}" in this fight. Use a name or numeric source id from Warcraft Logs.`,
            )
        }
        ...
        const [d1, t1, ms1] = await Promise.all([
            ...
            gql(
                `query($c:String!,$s:Float!,$e:Float!){reportData{report(code:$c){table(dataType:DamageTaken,startTime:$s,endTime:$e)}}}`,
                {
                    c: code,
                    s: fight1.startTime,
                    e: fight1.endTime,
        ...
        try {
            ...
            patchSession({
                ...
                compareName2: '',
                specId: specId ?? null,
                p1TalentTreeJson: talentDataToP1RowsJson(
                    tt1?.talentTree?.length ? tt1.talentTree : tt1?.talents,
                ),
            })
        } catch (e: any) {
    ...
    const loadCompare = useCallback(
        async (urlOverride?: unknown) => {
            // Guard the override: onClick handlers pass a MouseEvent here.
            const raw =
                typeof urlOverride === 'string' && urlOverride.trim() ? urlOverride.trim() : compareUrl.trim()
            if (!raw) {
            ...
            if (
                ...
                p1data &&
                loadedKeys &&
                (raw === loadedKeys.raw || raw === loadedKeys.canonical)
            ) {
            ...
            try {
                ...
                if (!(m1 as any).reportData?.report)
                    throw new Error(`Report ${r1} not found or inaccessible.`)
                if (!(m2 as any).reportData?.report)
                    throw new Error(`Report ${r2} not found or inaccessible.`)
                ...
                const [d1, d2, t1, t2, ms1, ms2] = await Promise.all([
                    ...
                    fetchMetricGraphs(gql, r1, fight1.startTime, fight1.endTime, name1).catch(
                        () => undefined,
                    ),
                    fetchMetricGraphs(gql, r2, fight2.startTime, fight2.endTime, name2).catch(
                        () => undefined,
                    ),
                ])
                const dmgE1 = (d1 as any)?.reportData?.report?.table?.data?.entries || []
                    ...
                    function resolveTalentNames(talentData: any) {
                        ...
                        return {
                            ...
                            talentTree: (talentData.talentTree || []).map((t: any) => ({
                                ...
                                id: t.spellId || t.id,
                                name:
                                    resolvedNames[t.spellId || t.id] ||
                                    t.name ||
                                    `Talent ${t.spellId || t.id}`,
                            })),
                        }
                    }
