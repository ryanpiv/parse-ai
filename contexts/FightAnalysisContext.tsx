...
export function FightAnalysisProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    if (analysisSubtab === 'compare' && (!p1data || !p2data)) setAnalysisSubtab('solo')
  }, [analysisSubtab, p1data, p2data])
  ...
  const runAI = useCallback(
    async (userMsg: string, priorThread: ChatMsg[], ctxOverride: string | undefined, channel: 'compare' | 'analyze') => {
      if (channel === 'compare') {
        if (!ctxOverride && (!p1data || !p2data)) return
      } else {
    ...
    [p1data, p2data, buildContextCompare, buildContextPlayerOneCb]
  )
  ...
  const sendCompareQuestion = useCallback(
    ...
    [aiLoading, p1data, p2data, inputCompare, messagesCompare, runAI]
  )
  ...
  }, [p1data, p2data, buildContextCompare])

  const loadCompare = useCallback(async () => {
    if (!compareUrl.trim()) {
      setStatus({ type: 'err', msg: 'Paste a WarcraftLogs compare URL.' })
      return
    }
    ...
    try {
      ...
      f1id = parseInt(fights[0])
      f2id = parseInt(fights[1] || fights[0])
      const srcs = (u.searchParams.get('source') || '').split(',')
      src1 = srcs[0]
      src2 = srcs[1] || srcs[0]
    } catch (e: any) {
      setStatus({ type: 'err', msg: 'Could not parse URL: ' + e.message })
      return
    }
