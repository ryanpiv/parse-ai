...
const SpellTimeline = ({
    ...
    const onWheel = useCallback(
        (e: React.WheelEvent) => {
            ...
            const panX = Math.abs(e.deltaX) > Math.abs(e.deltaY) ? e.deltaX : e.shiftKey ? e.deltaY : 0
            if (Math.abs(panX) > 1) {
                e.preventDefault()
                const cw = Math.max(120, containerW - LABEL_W)
            ...
            >
                <svg width={containerW} height={totalH} className={styles.svg}>
                            ...
                            return (
                                <g key={playerIdx}>
                                    <rect
                                        ...
                                        width={containerW}
                                        height={ROW_PLAYER}
                                        fill={(gi + playerIdx) % 2 === 0 ? BG_ALT : BG}
                                    />
                                    <line
                                        x1={LABEL_W}
                                        y1={mid}
                                                ...
                                                <g key={`${playerIdx}-${si}-${seg.tStart}`}>
                                                    ...
                                                    >
                                                        <title>
                                                            ...
                                                            {seg.cancelled
                                                                ? ' (cancelled / clipped)'
                                                                : ''} · {seg.tStart.toFixed(2)}s →{' '}
                                                            {seg.tEnd.toFixed(2)}s
                                                        </title>
                                                    </rect>
                                                </g>
                                            )
                    ...
                    {groups.map((g, gi) => {
                        const yBase = HEADER_H + gi * (blockH + GROUP_GAP)
                        const yRows = yBase + SPELL_LABEL_H
                        const spellLabel = g.name.length > 22 ? `${g.name.slice(0, 20)}…` : g.name
                        return (
                            <g key={`lab-${g.spellId}`}>
