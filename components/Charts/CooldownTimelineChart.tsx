...
function CooldownBarInner({
    ...
    const chartConfig = useMemo(() => {
        ...
        const estSec = (r: SpellRow) => estimatedCooldownSec(r, p1data.dur, solo ? p1data.dur : p2data.dur)
...
export function CooldownTimelineChart(props: {
    ...
    const effectiveCompareDur = Math.max(
        1,
        Math.min(
            Math.max(p1data.dur, p2data.dur),
            Number(compareWindowSec) || Math.max(p1data.dur, p2data.dur),
        ),
    )

    const idSig = useMemo(() => collectSpellIdsForApiLookup(spellRows, solo).join(','), [spellRows, solo])

    const [blizzardCooldownMs, setBlizzardCooldownMs] = useState<Record<string, number | null> | null>(null)
    ...
    useEffect(() => {
        ...
        ;(async () => {
            try {
                ...
                if (cancelled) return
                setBlizzardCooldownMs(
                    typeof data.cooldowns === 'object' && data.cooldowns ? data.cooldowns : {},
                )
            } catch {
