...
export function WclKeyPrompt() {
    ...
    if (fa.wclClientId) {
        return (
            ...
            >
                <div style={{ ...s.label, color: 'var(--gold2)', marginBottom: 8 }}>
                    WarcraftLogs sign-in required
                </div>
                <button
                    type="button"
            ...
            {showHelp && (
                ...
                >
                    <li>
                        ...
                        with redirect URL{' '}
                        <code style={{ color: 'var(--blue)' }}>http://localhost:3000/auth/callback</code> (add
                        your production URL too when deploying).
                    </li>
                    <li>
                        Add <code style={{ color: 'var(--blue)' }}>WCL_CLIENT_ID</code> to{' '}
                        <code style={{ color: 'var(--blue)' }}>.env.local</code> (or your host&apos;s
                        environment variables). Public/PKCE clients have no secret; add{' '}
                        <code style={{ color: 'var(--blue)' }}>WCL_CLIENT_SECRET</code> too only if your
