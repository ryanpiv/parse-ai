...
function useEntryIcon(spellId: number, talentId = 0): string | null {
    const [icon, setIcon] = useState<string | null>(() => {
        if (spellId > 0 && iconCache[`s:${spellId}`] !== undefined)
            return iconCache[`s:${spellId}`]
                ? `https://wow.zamimg.com/images/wow/icons/medium/${iconCache[`s:${spellId}`]}.jpg`
                : null
        if (spellId <= 0 && talentId > 0 && iconCache[`t:${talentId}`] !== undefined)
            return iconCache[`t:${talentId}`]
                ? `https://wow.zamimg.com/images/wow/icons/medium/${iconCache[`t:${talentId}`]}.jpg`
                : null
        return null
    })
...
const NodeIcon = ({ node, size, renderMode }: INodeIconProps) => {
    ...
    const diffStyle: Record<
        DiffState,
        { bg: string; border: string; opacity: number; borderWidth: number; boxShadow: string }
    > = {
        both: {
    ...
    const s = renderMode === 'raidbots' ? (active ? raidbotsActive : raidbotsInactive) : diffStyle[state]
    ...
    const containerStyle: React.CSSProperties =
        renderMode === 'raidbots'
            ? {
                  ...
                  flexShrink: 0,
                  boxShadow:
                      (s as typeof raidbotsActive).glow !== 'none'
                          ? `0 0 7px ${(s as typeof raidbotsActive).glow}, inset 0 0 0 1px ${(s as typeof raidbotsActive).glow}`
                          : 'none',
...
const ChoiceNodeIcon = ({ node, size, containerStyle, s, renderMode, active }: IChoiceNodeIconProps) => {
    ...
    const fallbackTitle = node.rank ? `${node.rank === 1 ? name0 : name1}` : `${name0} / ${name1}`
        ...
        <div style={{ position: 'relative', width: W, height: H, flexShrink: 0 }}>
            <svg
                ...
                style={{
                    ...
                    left: 0,
                    pointerEvents: 'none',
                    zIndex: renderMode === 'raidbots' ? 5 : undefined,
                }}
            >
                {sectionEdges.map((e, i) => {
                    ...
                    const lineState: DiffState =
                        p1Edge && p2Edge ? 'both' : p1Edge ? 'p1' : p2Edge ? 'p2' : 'neither'
                    ...
                    const color =
                        lineState === 'p1'
                            ...
                            : lineState === 'p2'
                              ? '#4cb0f0'
                              : lineState === 'both'
                                ? 'rgba(158,170,186,0.85)'
                                : 'rgba(88,100,118,0.5)'
                    const sw =
                        lineState === 'p1' || lineState === 'p2' ? 3.5 : lineState === 'both' ? 2.25 : 1.5
                    return (
                        <line
                            key={i}
                            x1={fp.x}
