...
function mockReq(overrides: Partial<NextApiRequest> = {}): NextApiRequest {
  return { method: 'POST', body: { model: 'claude-sonnet-4-6', messages: [] }, headers: {}, ...overrides } as NextApiRequest
}
...
describe('/api/ai', () => {
  ...
  it('returns 401 when API key is missing', async () => {
    ...
    expect(res.json).toHaveBeenCalledWith(expect.objectContaining({ error: expect.stringContaining('Claude API key') }))
  })

  it('prefers a user Console key header over the server env key', async () => {
    mockFetch.mockResolvedValueOnce({ status: 200, json: async () => ({ content: [{ text: 'ok' }] }) } as Response)
    const res = mockRes()
    await handler(
      mockReq({
