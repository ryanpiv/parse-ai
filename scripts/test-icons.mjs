...
// Print results table
console.log(['spellId', 'name', 'icon slug', 'img ok', 'error'].map((h) => h.padEnd(20)).join('| '))
...
if (passed < total) {
    console.log('\nFailed:')
    results
        .filter((r) => !r.ok || !r.imgOk)
        .filter((r) => r.spellId > 0 && r.spellId < 900000)
        .forEach((r) =>
            console.log(`  ${r.spellId} (${r.testName}): ${r.error || 'icon slug ok but image 404'}`),
        )
}
