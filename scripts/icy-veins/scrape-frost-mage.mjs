/**
 * Fetch Frost Mage Icy Veins **talents** and **rotation** pages only — the canonical
 * URLs for those topics — and write structured JSON under knowledge/icy-veins/scraped/.
 *
 ...
 *
 * Usage: node scripts/icy-veins/scrape-frost-mage.mjs
 *
...
const outDir = path.join(root, 'knowledge/icy-veins/scraped/mage-frost')

const UA = 'Mozilla/5.0 (compatible; ParseAnalyzerGuideSync/0.1; local-dev guide snapshot)'

/** Canonical IV URLs for Frost Mage talents vs rotation (not the generic spec hub page). */
const PAGES = [
    {
...
/**
 * @param {Record<string, unknown> | null} articleLd
 * @returns {string | null}
 */
...
function buildSnapshotMeta({ url, fetchResult, scrapedAt, articleLd, pageKind }) {
    ...
    const howTo = {
        refreshProcedure:
            'Re-run `npm run scrape-icy-veins-frost` when guides change; commit JSON under knowledge/icy-veins/scraped/.',
    }
    if (pageKind === 'talents') {
        howTo.talentEmbeds =
    ...
    return {
        ...
        fetch: {
            ...
            note: blocked ? 'Unusable snapshot — short body, challenge page, or error HTML.' : undefined,
        },
...
/**
 * @param {Array<{ title: string, url: string, current?: boolean }>} breadcrumbToc
 */
