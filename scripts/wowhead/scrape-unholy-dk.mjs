/**
 * Fetch Unholy Death Knight Wowhead guides and write structured JSON under knowledge/wowhead/scraped/.
 *
 * Usage: node scripts/wowhead/scrape-unholy-dk.mjs
 *
 * Important:
...
const outDir = path.join(root, 'knowledge/wowhead/scraped/death-knight-unholy')

const UA =
  'Mozilla/5.0 (compatible; ParseAnalyzerGuideSync/0.1; local-dev guide snapshot)'

const WOW_SPEC_ID = 252
...
function buildSnapshotMeta({
    ...
    try {
      ...
      if (h.specId !== expectedWowSpecId) {
        talentHeaderVsGuideWarning =
          `parseTalentStringHeader on this Wowhead export yields specId ${h.specId}, not ChrSpecialization id ${expectedWowSpecId} for Unholy DK. Treat header fields as raw Blizzard bit layout — see __tests__/lib/talents/decodeTalentString.ts (Wowhead CAE sample).`
      }
    } catch {
      /* ignore */
    }
  ...
  return {
    ...
    howToStayCurrent: {
      refreshProcedure:
        'Re-run `npm run scrape-wowhead-unholy` when guides change; commit new JSON. Wowhead "Updated" date + sidebar patch are the closest HTML-native freshness signals.',
      notInHtml:
