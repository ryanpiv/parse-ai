    ...
    if (checkSpecId && talentCodes.length > 0) {
        try {
            ...
            if (h.specId !== spec.wowSpecId) {
                talentHeaderVsGuideWarning = `parseTalentStringHeader on this Wowhead export yields specId ${h.specId}, not ChrSpecialization id ${spec.wowSpecId} for ${spec.label}. Treat header fields as raw Blizzard bit layout — see __tests__/lib/talents/decodeTalentString.ts (Wowhead CAE sample).`
            }
        } catch {
            /* ignore */
        }
    ...
    for (const page of pagesFor(spec)) {
        ...
        if (page.key === 'talent-builds') {
            ...
            console.log(
                'Wrote',
                folder,
                'talent-builds.json — copy rows:',
                talent.copies.length,
                'codes:',
                payload.uniqueImportCodes.length,
            )
        }
        ...
        if (page.key === 'rotation-cooldowns') {
            ...
            fs.writeFileSync(
                path.join(outDir, 'rotation-cooldowns.json'),
                JSON.stringify(payload, null, 2) + '\n',
            )
            console.log(
                'Wrote',
                folder,
                'rotation-cooldowns.json — sections:',
                sections.length,
                'markup chars:',
                markup.length,
            )
