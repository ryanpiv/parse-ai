...
export function extractSidebarPatchLabel(html) {
    const m = html.match(/interior-sidebar-header-text-subtitle[^>]*>\s*Patch\s+([\d.]+)\s*</i)
    if (!m) return null
    return {
        label: `Patch ${m[1]}`,
        patchSemverLike: m[1],
...
export function extractGuideNumericId(html) {
    ...
    if (m2)
        return { guideId: Number(m2[1]), sourcedFrom: 'Wowhead HTML: `WH.Favorites.pageInit(..., guideId)`' }
    return null
}
...
export function extractBbCodeSeasonBanner(markup) {
    ...
    return {
        phrase: m[1],
        sourcedFrom: 'Wowhead BBCode: bracket emphasis block `Midnight Season N` in guide-body article text',
    }
}
