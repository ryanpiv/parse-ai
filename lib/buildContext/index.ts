import { fmtOverall, fmtBuffUptimes, fmtCastDetails, fmtPotion } from './formatters'
...
export function buildRichContext(
    ...
    ;[...allSpells].forEach((s) => {
        try {
            ...
            if (name && !name.startsWith('Spell '))
                ctx += `${name}: spell ID ${id} → https://www.wowhead.com/spell=${id}\n`
        } catch {}
    })
    ctx += '\n'
    ...
    if (talentDiff?.t1 || talentDiff?.t2) {
        ...
        const ids1 = new Set(
            (talentDiff.t1?.talentTree || talentDiff.t1?.talents || []).map((t: any) => t.id),
        )
        const ids2 = new Set(
            (talentDiff.t2?.talentTree || talentDiff.t2?.talents || []).map((t: any) => t.id),
        )
        const all = new Map()
        ;(talentDiff.t1?.talentTree || talentDiff.t1?.talents || []).forEach((t: any) => all.set(t.id, t))
        ;(talentDiff.t2?.talentTree || talentDiff.t2?.talents || []).forEach((t: any) => {
    ...
    ctx += `${'-'.repeat(80)}\n`
    p1.spellRows
        .filter((r: any) => r.ppm1 > 0 || r.ppm2 > 0)
        .forEach((r: any) => {
    ...
    p1.spellRows
        .filter((r: any) => r.ppm1 > 0.3 || r.ppm2 > 0.3)
        .slice(0, 10)
        .forEach((r: any) => {
            ctx += `${r.name}:\n`
...
export function buildRichContextPlayerOne(
    p1: any,
    talentDiff: any,
    options: BuildPlayerOneContextOptions,
): string {
    const { name: n1, spec: s1 } = p1
    ...
    if (npcDeaths.length) {
        ctx += `Count: ${npcDeaths.length} — times from pull (s): ${npcDeaths
            .slice(0, 25)
            .map((t) => t.toFixed(1))
            .join(', ')}${npcDeaths.length > 25 ? ' …' : ''}\n\n`
    } else {
