...
function seriesPoints(series: any, fightStart: number): MetricPoints {
  ...
  if (Array.isArray(arr[0])) {
    return arr.map((p: any) => [(Number(p[0]) - fightStart) / 1000, Number(p[1]) || 0] as [number, number])
  }
  const start = Number(series?.pointStart ?? fightStart)
  const interval = Number(series?.pointInterval ?? 1000)
  return arr.map((v: any, i: number) => [(start + i * interval - fightStart) / 1000, Number(v) || 0] as [number, number])
}
